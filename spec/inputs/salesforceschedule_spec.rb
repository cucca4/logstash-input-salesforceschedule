  ...
  describe "inputs/salesforce" do
    ...
    context "add fields and filters" do
      ...
      it "should build a query" do
        expect(subject.send(:get_query)).to eq('SELECT Something FROM Case')
      end
    end
    context "describe Case object" do
      ...
      let(:expected_types_result) { [["FirstName", "string"],
                                     ...
                                     ["Salutation", "picklist"]] }
      subject { input }
      it "loads the Case object fields" do
        VCR.use_cassette("describe_case_object",:decode_compressed_response => true) do
          subject.register
      ...
      context "load Case objects" do
        ...
        subject { input }
        let(:queue) { [] }
        it "loads some case records" do
          VCR.use_cassette("load some case objects",:decode_compressed_response => true) do
            subject.register
            subject.run(queue)
